...
function AddNotf() {
  ...
  const submitHandler = async (event) => {
    ...
    const file = await NotfService.uploadFile(event.target.file.files[0])

    const title = event.target.title.value
    const fileId = file.$id

    NotfService.createNotf({title, fileId})
  ...
  return (
    ...
    <form onSubmit={submitHandler}
      className='flex flex-col max-w-lg bg-[#11182e] gap-5 p-10 text-white rounded-3xl'>
      ...
      <div className='flex gap-5'>
        <label htmlFor="file" className='text-nowrap'>Upload File :</label>
